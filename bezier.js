...
Bezier.prototype._DIV = 100;

Bezier.prototype._computeArcLengths = function () {
  this._arcLengths = new Array(this._DIV + 1);
  this._arcLengths[0] = 0;
  
  var curr = this.compute(0),
      clen = 0;
  
  for (var i = 1; i <= this._DIV; i += 1) {
    var next = this.compute(i / this._DIV),
        dx = next.x - curr.x,
        dy = next.y - curr.y;
...
var _oldUpdate = Bezier.prototype.update;
Bezier.prototype.update = function () {
  _oldUpdate.call(this);
  this._computeArcLengths();
};

Bezier.prototype.mapu = function (u) {
  var targetLength = u * this._arcLengths[this._DIV];
  var low = 0, high = this._DIV, index = 0;
  while (low < high) {
  ...
  var lengthBefore = this._arcLengths[index];
  if (lengthBefore === targetLength) {
    return index / this._DIV;
  } else {
    return (index + (targetLength - lengthBefore) / (this._arcLengths[index + 1] - lengthBefore)) / this._DIV;
  }
};
...
// Patch: derivative not defined for linear curve
var _oldDeriv = Bezier.prototype.derivative;
Bezier.prototype.derivative = function (t) {
  return _oldDeriv.call((this.order == 1)
    ? new Bezier([
        this.points[0],
        {
...
InterpolatedPath.prototype.closest = function (toPoint) {
  ...
  if (best) {
    ...
    best.proj = this.curves[best.curve_i].compute(best.t);
    best.curve = this.curves[best.curve_i];
  }
  return best;
};
